...
export function S21_About() {
	...
	return (
		<Page tag="17 · 关于课代表" title={<>7 年+ 的<span style={{ color: colors.purple }}>大学生品牌矩阵</span></>}
			...
			<div style={{ display: 'grid', gridTemplateColumns: '1.35fr 0.65fr', gap: 22, marginBottom: 20, alignItems: 'stretch' }}>
				<Editorial tone="dark" accent={colors.purple} size={26}>
					课代表系列由匠人学院自 <b style={{ color: colors.yellow }}>2017 年</b> 创建，聚焦大学生群体，7 年+ 运营，打造 <b style={{ color: colors.yellow }}>8 个小红书账号 + 6 个微信公众号</b> 矩阵及活跃线下社区，已成为澳洲留学生广泛认可的品牌。
				</Editorial>
				<Photo img="past-events/crowd-stage.jpg" caption="线下社区现场" style={{ minHeight: 200 }} />
			</div>
...
export function S22_Xhs() {
	...
	return (
		<Page tag="18 · 小红书" title="课代表系列小红书" bg={colors.red} tone="dark" accent={colors.dark} watermark="书" align="center">
...
export function S23_XhsDesc() {
	return (
		<Page tag="18 · 小红书" title="四城八校的内容生态" accent={colors.red} watermark="XHS" align="center">
			<Editorial accent={colors.red} size={28}>
				借助小红书，「课代表系列」打造了囊括澳大利亚<b style={{ color: colors.red }}>四城八所顶级高校</b>的内容生态——墨尔本大学、Monash、RMIT、UQ、QUT、USYD、UNSW、阿德莱德，每校设代表性账户，分享大学生活、教育资讯、本地活动。与之合作，您的品牌可<b style={{ color: colors.red }}>精准接触活跃、具消费潜力的年轻群体</b>，自然融入大学生日常，有效提升品牌知名度。
			</Editorial>
		</Page>
	);
}
...
export function S24_XhsMatrix() {
	return (
		<Page tag="18 · 小红书" title="账号矩阵数据" accent={colors.red} watermark="4★" align="center">
...
export function S25_Wechat() {
	...
	return (
		<Page tag="19 · 公众号" title="课代表系列微信公众号" bg={colors.green} accent={colors.dark} watermark="W" align="center">
...
export function S26_WechatDesc() {
	return (
		<Page tag="19 · 公众号" title="高忠诚度的大学生社区" accent={colors.green} watermark="WX" align="center">
...
export function S27_WechatMatrix() {
	return (
		<Page tag="19 · 公众号" title="公众号矩阵数据" accent={colors.green} watermark="6" align="center">
			<KvTable head={['公众号', '粉丝', '平均曝光']} rows={[['墨大课代表', '8200+', '800+'], ['Monash 课代表', '2900+', '200+'], ['UQ 课代表', '14000+', '2000+'], ['USYD 课代表', '6200+', '700+'], ['UNSW 课代表', '5878+', '500+'], ['阿德课代表', '3200+', '200+']]} />
		</Page>
	);
}

export function S28_Community() {
	const stats = [['1000+', '小红书活跃大学生社群用户'], ['10,000+', '高粘性微信大学生社群用户']] as const;
	const palette = [colors.yellow, colors.white];
	return (
		<Page tag="20 · 课代表社群" title="可直达的私域社群" bg={colors.blue} tone="dark" accent={colors.dark} watermark="@" align="center">
...
export function S29_Coop() {
	return (
		<Page tag="21 · 合作机会" title={<>深入高校市场的<span style={{ color: colors.yellow }}>桥梁</span></>}
