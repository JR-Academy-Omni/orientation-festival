	...
	return (
		<PhotoSlide img="past-events/consulting-table.jpg">
			...
			<h1 style={{ fontFamily: fonts.heading, fontWeight: 900, color: colors.white, fontSize: 76, lineHeight: 1.07, letterSpacing: -1, marginBottom: 16, maxWidth: 1120 }}>
				Put your brand in front of<br /><span style={{ display: 'inline-block', marginTop: 8, background: colors.red, color: colors.white, padding: '0 20px', border, boxShadow: shadow }}>every first-year's first week</span>
			</h1>
			<p style={{ fontSize: 25, fontWeight: 800, color: colors.white, marginBottom: 26 }}>We'll tailor a partnership package for you and your brand</p>
			<motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }}
				...
				<div style={{ padding: '24px 28px', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 8 }}>
					...
					<div style={{ fontSize: 20, fontWeight: 700, color: colors.dark }}>business@jiangren.com.au</div>
				</div>
			</motion.div>
		</PhotoSlide>
	);
